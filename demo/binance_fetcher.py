...
def fetch_klines(symbol, interval, start_time_ms, end_time_ms):
...
def fetch_klines_since(symbol, interval, since_ms):
    ...
    return fetch_klines(symbol, interval, start_ms, end_time_ms)
...
def get_last_30_days(symbol="ETHUSDT", interval="15m"):
    """Fetch last 30 days of K-lines for a symbol."""
    end_time_ms = int(time.time() * 1000)
    start_time_ms = end_time_ms - 30 * 24 * 60 * 60 * 1000

    raw = fetch_klines(symbol, interval, start_time_ms, end_time_ms)
